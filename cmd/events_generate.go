...
// eventSyncRunCache is the per-`chb calendars sync` run cache for OG image
// metadata. It's used during the generate phase (image enrichment) even
// though the command is named "sync" — the cache lives as long as a single
// invocation of the calendars-sync command.
